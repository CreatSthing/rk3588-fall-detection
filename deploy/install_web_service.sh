...
data_dir=/var/lib/rk3588-camera
...
install -d -m 0755 "$etc_dir" "$data_dir" "$data_dir/events" /var/log/rk3588-camera
chown -R "$service_user:$service_group" "$data_dir" /var/log/rk3588-camera
...
if grep -q '^RK3588_WEB_CONFIG=' "$etc_dir/web.env"; then
    sed -i 's|^RK3588_WEB_CONFIG=.*|RK3588_WEB_CONFIG=/var/lib/rk3588-camera/web.json|' "$etc_dir/web.env"
else
    printf '%s\n' 'RK3588_WEB_CONFIG=/var/lib/rk3588-camera/web.json' >> "$etc_dir/web.env"
fi

if [ ! -f "$data_dir/web.json" ]; then
    if [ -f "$etc_dir/web.json" ]; then
        install -m 0640 "$etc_dir/web.json" "$data_dir/web.json"
    else
...
model_path="$app_root/assets/weights/yolov8n-pose-int8-calibrated-20260818.rknn"
if [ ! -s "$model_path" ]; then
    echo "Missing pose model: $model_path" >&2
    exit 5
fi
