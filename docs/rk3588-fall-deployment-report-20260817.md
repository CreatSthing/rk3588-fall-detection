...
- 表现：同步骨架后画面虽然和关键点匹配，但已经落后实时约6分钟，并且延迟继续增加。
- 原因：FP16 推理约11 FPS，输入流帧率更高；原实现直接从 FFmpeg stdout FIFO 逐帧读取，没有丢帧机制。
- 解决：仅对网络流启动持续排空线程，覆盖保存最新完整帧，推理线程取最新帧并统计 `source_frames_dropped`；文件回放仍逐帧处理。FFmpeg 同时启用 `nobuffer` 和 `low_delay`。事件录像按采样时间计算实际帧率，避免主动丢帧后录像播放加速。
- 验证：23秒内处理203帧并主动跳过131个过期帧，证明消费速度低于输入时旧帧会被抛弃而不再排队；服务持续运行且 `last_error` 为空。

### 4.13 刷新后人员框正确但骨架仍偏移

- 表现：截图中绿色框正确框住真人，蓝色骨架却整体留在左侧。
- 原因：模型原始 JSON 的框和关键点实际一致；前端把不含同步 JPEG 的 `/api/status` 结果叠在独立 WebRTC 画面上，并可能复用上一次管线的 SVG 节点。
- 解决：前端现在只绘制带同帧 JPEG 的检测结果；管线重启清空成对缓存；JPEG 解码完成后原子更新；SVG/框按 `frame_id` 强制重建；首页禁用缓存。
- 验证：浏览器端实际截图中跌倒人体框为 `x=280.7～527.1`，关键点为 `x=299～503`，骨架与人体正确重合。验证后恢复实时 RTSP，测试产生的两条离线告警已确认，未删除录像或事件记录。
