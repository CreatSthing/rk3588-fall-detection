...
- 当前发布：`/opt/rk3588-camera/releases/20260817-fall-01/package`。
...
最终运行快照：服务连续处理6928帧时为19.93 FPS，20秒内帧数增加401；systemd重启次数为0，算法进程RSS约397 MiB，SoC最高约50.8°C，`last_error`为空。该快照只证明短时运行正常，不能替代后续24～72小时长稳测试。

当前摄像头画面是空床位，没有人员，因此没有做真人模拟跌倒。已分别验证“真实 NPU 姿态识别”“时序跌倒状态机”和“板端告警录像闭环”，但正式上线前仍必须录制现场跌倒、坐下、弯腰、躺床等视频做误报/漏报标定。
...
- 量化数据：当前安装摄像头画面40张。
- RKNN SHA-256：`52fb616049eeaea318242ee7972a65d3da7e44cd3d0e9ab8142678dd28523cc3`。
