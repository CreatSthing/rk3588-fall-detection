...
- 初版骨架叠加在 MediaMTX WebRTC iframe 外面，视频延迟和 iframe 内部缩放会造成错位。现已改为同一推理帧的 JPEG 和关键点成对推送，静态资源版本更新为 `pose-sync-20260817-3`。同步样本保存在本地 `.runtime/evidence/20260817-pose-sync/`。
- 随后发现初版 INT8 的人体置信度全部卡在 `0.500`：40张量化图没有人体，导致目标分支范围截断，手部也可能被误检。生产模型切换为 FP16（SHA-256 `3d63f96d...793c17`），同一跌倒帧置信度恢复到 `0.8106`。FP16 端到端复测告警置信度为 `0.82`，录像484036字节且状态为 `ready`；同步帧和检测 JSON 保存在 `.runtime/evidence/20260817-pose-fp16/`。
